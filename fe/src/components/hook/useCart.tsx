...
const useCart = () => {
    ...
    const { data, isLoading, isError, error, refetch } = useQuery({
        ...
        queryFn: async () => {
            ...
            try {
                ...
                console.log('Response từ API:', response.data); // Ghi log phản hồi API
    ...
    const { mutate: updateCart } = useMutation({
        mutationFn: async ({ action, productId, quantity }: { action: string; productId: string; quantity?: number }) => {
            try {
                switch (action) {
                    case "INCREASE":
                        await instanceClient.put(`/gio-hang/${productId}`, { so_luong: quantity ? quantity + 1 : 1 });
                        break;
                    case "DECREASE":
                        if (quantity && quantity > 1) {
                            await instanceClient.put(`/gio-hang/${productId}`, { so_luong: quantity - 1 });
                        } else {
    ...
    const calculateTotal = () => {
        ...
        return data.products.reduce((total: number, product: { gia: number; so_luong: number }) => total + product.gia * product.so_luong, 0);
    };
