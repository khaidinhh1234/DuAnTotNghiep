...
//       await instanceClient.delete(`/gio-hang/${id}`, {
//         headers: {
//           Authorization: `Bearer ${access_token}`,
//         },
//       });
...
//     if (isChecked) {
//       const allProductIds = [
//         ...(cartData?.san_pham_giam_gia?.map((product: any) => product.id) || []),
//         ...(cartData?.san_pham_nguyen_gia?.map((product: any) => product.id) || []),
//       ];
...
//         ...(cartData.san_pham_giam_gia?.filter((p: any) => p.chon === 1) || []).map((p: any) => p.id),
//         ...(cartData.san_pham_nguyen_gia?.filter((p: any) => p.chon === 1) || []).map((p: any) => p.id)
//       ];
