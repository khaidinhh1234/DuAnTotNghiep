...
function CreditCardForm({ bankData }: { bankData: BankData }) {
  ...
  const handleCardHolderChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!nameRegex.test(input) && input !== "") {
      setError1("Tên chủ tài khoản không hợp lệ");
      setCardHolder(input); // Vẫn set giá trị nếu người dùng nhập sai để họ chỉnh sửa
      return;
    }
    ...
    if (input.trim().length < 10) {
      ...
      setError1(""); // Xóa lỗi nếu nhập đúng
    }
      ...
      <div className="rounded-2xl bg-white p-6 shadow-lg">
        <div className="mb-6">
          <div
            ...
            style={{
              backgroundImage: `url("${bankImages[bankData.name] || "/istockphoto-1332736514-1024x1024.jpg"}")`,
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="mt-1 w-full rounded-md border border-gray-300 p-2.5 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 transition duration-300 ease-in-out transform hover:scale-105"
            />
            {error1 && <p className="mt-1 text-sm text-red-500">{error1}</p>}
          </div>
