...
//             }
//           } else if (img.url) {
//             formData.append(`bien_the[${index}][anh][${imgIndex}]`, img.url);
//           }
//         }
//       } else {
...
const EditProductsAndVariants: React.FC = () => {
  ...
  const prepareFormData = async () => {
    ...
    if (productFormData.tags && productFormData.tags.length > 0) {
      productFormData.tags.forEach((tagId: number, index: number) => {
        formData.append(`the[${index}]`, tagId.toString());
        console.log(`Adding the[${index}]:`, tagId.toString());
      });
    } else {
      formData.append('the', '[]');
      console.log("Adding empty the array");
    }
    ...
    if (fileList.length > 0 && fileList[0].originFileObj) {
      try {
        const imageUrl = await uploadToCloudinary(fileList[0].originFileObj as RcFile);
        formData.append('anh_san_pham', imageUrl);
        console.log("Adding anh_san_pham:", imageUrl);
      } catch (error) {
        console.error('Error uploading product image:', error);
        throw new Error('Lỗi khi tải lên ảnh sản phẩm');
      }
      ...
      console.log("Adding existing anh_san_pham:", productData.data.anh_san_pham);
    }
    ...
    if (variantData.length > 0) {
      variantData.forEach((variant, index) => {
        Object.entries(variant).forEach(([key, value]) => {
          if (key !== 'anh_bien_the' && value !== undefined && value !== null) {
            formData.append(`bien_the[${index}][${key}]`, value.toString());
            console.log(`Adding bien_the[${index}][${key}]:`, value.toString());
          }
        });
  
        // Add variant images
        if (variant.anh_bien_the && variant.anh_bien_the.length > 0) {
          variant.anh_bien_the.forEach((img, imgIndex) => {
            if (img.url) {
              formData.append(`bien_the[${index}][anh][${imgIndex}]`, img.url);
              console.log(`Adding bien_the[${index}][anh][${imgIndex}]:`, img.url);
            }
          });
        }
      });
    } else {
      formData.append('bien_the', '[]');
      console.log("Adding empty bien_the array");
    }
