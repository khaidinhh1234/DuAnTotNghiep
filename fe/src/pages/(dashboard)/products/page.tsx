...
//     },
//     {
//       title: "Lượt xem", // Cột hiển thị lượt xem
//       dataIndex: "luot_xem",
//       key: "luot_xem",
//       width: "15%",
//       render: (text) => (text ? `${text} lượt` : "Chưa có lượt xem"),
//     },
//     {
//       title: "Mô tả ngắn",
...
//             okText="Có"
//             cancelText="Không"
//             onConfirm={() => console.log("Xóa sản phẩm:", record.key)}
//           >
...
import React, { useRef, useState } from "react";
import { DeleteOutlined, SearchOutlined, ReloadOutlined } from "@ant-design/icons";
import { Button, Input, Popconfirm, Space, Table, Switch } from "antd";
import Highlighter from "react-highlight-words";
import { Link } from "react-router-dom";
import { useQuery } from "@tanstack/react-query";
import instance from "@/configs/axios";
...
interface DataType {
  ...
  mo_ta_ngan: string;
  noi_dung: string;
  dang_hoat_dong: boolean; // Thêm trường trạng thái
}
...
const ProductsAdmin: React.FC = () => {
  ...
  const sanpham = data?.data.map((item: any, index: number) => ({
    ...
    index,
    ten_danh_muc: item.danh_muc ? item.danh_muc.ten_danh_muc : "Không có danh mục",
    dang_hoat_dong: item.dang_hoat_dong || false, // Thêm trạng thái vào dữ liệu
  }));
  ...
  const columns: TableColumnsType<DataType> = [
    ...
    },
    {
      ...
      render: (text, item) => (
        <Switch
          checked={text}
          onChange={() => console.log("Cập nhật trạng thái:", item.key)}
        />
      ),
    },
    {
