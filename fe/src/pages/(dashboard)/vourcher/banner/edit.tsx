...
//     queryFn: async () => {
//       const response = await instance.get("/chuongtrinhuudai");
...
//       });
//       setSelectedProducts(data.san_pham);
//     }
...
//       const response = await instance.put("/chuongtrinhuudai", chuongTrinhUuDai);
//       return response.data;
//     },
//     onSuccess: () => {
...
const ChuongTrinhUuDaiEdit: React.FC = () => {
  ...
  useEffect(() => {
    if (chuongTrinhUuDaiData?.data && !isLoadingChuongTrinh) {
      ...
      form.setFieldsValue({
        ...
        loai: data.loai,
        san_pham: data.san_pham,
        imageFile: data.duong_dan_anh ? [
          {
            uid: '-1',
            name: 'image.png',
            status: 'done',
            url: data.duong_dan_anh,
          },
        ] : [],
      });
      setSelectedProducts(data.san_pham);
  ...
  const onFinish = async (values: any) => {
    try {
      let imageUrl = chuongTrinhUuDaiData?.data?.duong_dan_anh || '';
      if (values.imageFile && values.imageFile[0] && values.imageFile[0].originFileObj) {
        imageUrl = await uploadToCloudinary(values.imageFile[0].originFileObj);
      }
  ...
  const getSelectedProductNames = () => {
    ...
    return selectedProducts.map(id => 
      allProductsData.data.find(product => product.id === id)?.ten_san_pham
    ).filter(Boolean);
  };
