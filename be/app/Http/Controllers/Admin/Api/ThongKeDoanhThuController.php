...
class ThongKeDoanhThuController extends Controller
{
    ...
    public function thongKeDoanhThuTuanTu(Request $request)
    {
        ...
        // Khởi tạo mảng chứa doanh thu
        $doanhThu = [
            'tong_doanh_thu_nam' => 0,
            'tong_doanh_thu_quy' => 0,
            'tong_doanh_thu_thang' => 0,
            'tong_doanh_thu_tuan' => 0,
        ...
        if ($nam) {
            $query->whereYear('ngay_giao_hang_thanh_cong', $nam);
            $doanhThu['tong_doanh_thu_nam'] = $query->sum('tong_tien_don_hang');
        ...
        if ($quy && $nam) {
            $query->whereRaw('QUARTER(ngay_giao_hang_thanh_cong) = ?', [$quy]);
            $doanhThu['tong_doanh_thu_quy'] = $query->sum('tong_tien_don_hang');
        ...
        if ($thang && $nam && $quy) {
            $query->whereMonth('ngay_giao_hang_thanh_cong', $thang);
            $doanhThu['tong_doanh_thu_thang'] = $query->sum('tong_tien_don_hang');
        ...
        if ($tuan && $nam && $thang && $quy) {
            $query->whereRaw('WEEK(ngay_giao_hang_thanh_cong, 1) = ?', [$tuan]);
            $doanhThu['tong_doanh_thu_tuan'] = $query->sum('tong_tien_don_hang');
        ...
        if ($ngay && $nam && $thang && $tuan && $quy) {
            $query->whereDay('ngay_giao_hang_thanh_cong', $ngay);
            $doanhThu['tong_doanh_thu_ngay'] = $query->sum('tong_tien_don_hang');
            $doanhThu['theo_ngay']['ngay'][] = $ngay;
            $doanhThu['theo_ngay']['doanh_thu'][] = $doanhThu['tong_doanh_thu_ngay'];
        }
