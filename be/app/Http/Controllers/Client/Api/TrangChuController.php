...
class TrangChuController extends Controller
{
    ...
    public function index()
    {
            ...
            ->with([
                'sanPhams' => function ($query) {
                    $query->select(
                        ...
                        '),
                                    DB::raw('
                            COALESCE(bien_the_san_phams.gia_khuyen_mai_tam_thoi, bien_the_san_phams.gia_khuyen_mai, bien_the_san_phams.gia_ban) as gia_hien_tai
                        '),
                                    DB::raw('
                            (CASE
                                WHEN EXISTS (
                                    SELECT 1
                                    FROM chuong_trinh_san_pham
                                    WHERE san_pham_id = bien_the_san_phams.san_pham_id
                                )
                                THEN "Đang trong chương trình ưu đãi"
                                ELSE null
                            END) as trong_chuong_trinh_uu_dai
                        ')
                                )
                                    ->join('bien_the_mau_sacs', 'bien_the_san_phams.bien_the_mau_sac_id', '=', 'bien_the_mau_sacs.id')
                                    ->join('bien_the_kich_thuocs', 'bien_the_san_phams.bien_the_kich_thuoc_id', '=', 'bien_the_kich_thuocs.id')
                                    ->leftJoin('anh_bien_thes', 'bien_the_san_phams.id', '=', 'anh_bien_thes.bien_the_san_pham_id');
                            }
                        ]);
                }
            ])
        ...
        $dataDanhGiaKhachHang = DanhGia::query()->with('user')->whereIn('so_sao_san_pham', [4, 5])->orderByDesc('id')->take(8)->get();
